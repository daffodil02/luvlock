...
1.  **Components (JSX):** Writing HTML inside JavaScript.
    - _Goal:_ Learn how to pass "props" (data) into a component to make it reusable.
2.  **`useState` (Memory):** How React remembers things.
    - _Goal:_ Understand how modifying a State variable (like `searchTerm` in our app) automatically forces the screen to update.
3.  **`useEffect` (Side Effects):** How React talks to the outside world.
    - _Goal:_ Learn how to use this to run code _once_ when a page first loads (like how we fetched your Google Sheet data).
...
- _Goal:_ See how we use `searchTerm.startsWith('#')` in `Dashboard.jsx`. This is a classic "Conditional Flow" that makes your app feel like a real tool!
...
1.  **Don't memorize it:** Keep the [Tailwind Documentation](https://tailwindcss.com/) open in a tab. Every developer searches things like _"tailwind how to make a circle"_ all day long.
...
## 🌟 5️⃣ Intermediate Concepts: The Upgrade (New!) 🕵️‍♀️🎯
...
- **Color Normalization**: Every status now belongs to a specific CSS class. If a status is `CANCELLED`, it gets a specific Red theme. This makes the UI predictable and calm.
