...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (codeFilter !== "all")
      filtered = filtered.filter((o) => (o.TAG || o.CODE) === codeFilter);
  ...
  const handleSearch = (e) => {
    ...
    const results = orders.filter((order) => {
      ...
      if (isBatchSearch)
        return (order.TAG || order.CODE)?.toLowerCase().includes(term);
  ...
  const uniqueStatuses = [
    ...new Set(searchResults.map((o) => o.STATUS).filter(Boolean)),
  ];
  const uniqueCodes = [
    ...new Set(searchResults.map((o) => o.TAG || o.CODE).filter(Boolean)),
  ];
  ...
  const formatTagWithFlag = (tag) => {
    ...
    if (!tag || tag.toUpperCase() === "N/A")
      return (
        <span className="text-gray-300 opacity-40 italic tracking-widest">
          --
        </span>
      );
      ...
      if (flags[countryCode]) {
        return (
          <span className="inline-flex items-center gap-1.5 align-middle">
            <span className="text-[17px] leading-none mb-0.5">
              {flags[countryCode]}
            </span>
            <span className="uppercase text-pink-600 dark:text-pink-400 font-bold tracking-tighter">
              #{restOfTag}
            </span>
          </span>
        );
  ...
  const arrivedItems = searchResults
    .filter((o) => {
      const s = o.STATUS?.toLowerCase() || "";
      return (
        s.includes("arrived pa") || s.includes("ready") || s.includes("shipped")
      );
    })
    .reduce((sum, o) => {
      const qtt = parseInt(o.QTT);
  ...
  const arrivalProgress =
    totalItems > 0 ? Math.round((arrivedItems / totalItems) * 100) : 0;
  const otwItems = totalItems - arrivedItems;
  const isBatchMode =
    searchTerm.trim().startsWith("#") && searchResults.length < orders.length;
  ...
  const getSmartDate = (obj, keyword) => {
    ...
    if (!val || val.trim() === "-" || val.trim().toLowerCase() === "tba")
      return "-";
  ...
  const PaymentStatus = ({ status, isLC }) => {
    if (isLC)
      return (
        <div
          className="w-3 h-3 rounded-full bg-gray-200 dark:bg-gray-700 mx-auto opacity-50"
    ...
    return (
      <div
        className={`w-3 h-3 rounded-full mx-auto shadow-sm transition-all duration-500 ${isPaid ? "bg-green-500 shadow-green-200 dark:shadow-green-900/20" : "bg-red-400 shadow-red-200 dark:shadow-red-910/20"}`}
        title={status || "Unpaid"}
      />
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-pink-500 rounded-3xl p-6 md:p-8 text-white shadow-xl shadow-pink-200/80 dark:shadow-none relative overflow-hidden">
        <div className="absolute top-[-10%] right-[-5%] opacity-15 text-9xl transform rotate-12">
          🔒
        </div>
        <div className="absolute bottom-[-10%] left-[-5%] opacity-15 text-9xl transform -rotate-12">
          💖
        </div>
        <div className="relative z-10">
          <h1 className="text-3xl font-extrabold mb-2 text-white uppercase tracking-tight font-header">
            Masterlist 🎀
          </h1>
          <p className="text-pink-100 font-medium text-xs uppercase tracking-wider opacity-90 mb-4">
            Your items are making their way to you!
          </p>
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Info size={14} />{" "}
              <span>{showGuide ? "Close Guide" : "Portal Guide"}</span>
            </button>
          </div>
        </div>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-0.5">
                <h4 className="text-[11px] font-black uppercase text-pink-500 tracking-wider">
                  1. Masterlist
                </h4>
                <p className="text-[10px] text-gray-400 dark:text-gray-300 font-bold leading-tight">
                  Track your order here. Seach your @username or #tag to see
                  your order status.
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-0.5">
                <h4 className="text-[11px] font-black uppercase text-blue-500 tracking-wider">
                  2. Ongoing GOs
                </h4>
                <p className="text-[10px] text-gray-400 dark:text-gray-300 font-bold leading-tight">
                  Check out new GOs available, join them and secure your claims!
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-0.5">
                <h4 className="text-[11px] font-black uppercase text-purple-500 tracking-wider">
                  3. Postage
                </h4>
                <p className="text-[10px] text-gray-400 dark:text-gray-300 font-bold leading-tight">
                  Items arrived to PA? That means your items are ready to be
                  shipped! Click the Postage page to checkout for delivery.
                </p>
              </div>
            </motion.div>
          </motion.div>
      ...
      <div className="glass rounded-3xl p-5 md:p-6 space-y-4">
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-2">
            <label className="text-sm font-bold text-pink-500 dark:text-pink-400 ml-1">
              Search Username or Tag
            </label>
            <form
            ...
            >
              ...
              <button
                type="submit"
                className="group absolute right-2 bg-pink-500 hover:bg-pink-600 text-white p-2 rounded-xl transition-all active:scale-95"
              >
                <Search
                  size={18}
          ...
          {searchTerm && (
            <button
              ...
              className="p-3 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-2xl hover:bg-gray-200 dark:hover:bg-gray-700 active:scale-95 shadow-sm"
            >
              <X size={20} />
            </button>
          )}
        ...
        {searchTerm && isSearched && (
          ...
          >
            <div className="flex-1 min-w-[140px] space-y-1.5">
              <label className="text-[10px] font-bold uppercase text-pink-400 ml-1">
                Status
              </label>
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="w-full bg-white dark:bg-pink-950/20 border border-pink-200 dark:border-pink-700/50 rounded-xl px-3 py-2 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-pink-400"
              >
            ...
            <button
              ...
              className="self-end px-4 py-2 text-xs font-bold text-pink-500 hover:text-pink-600 transition-colors uppercase"
            >
              Reset All
            </button>
          </motion.div>
        ...
        {!searchTerm || !isSearched ? null : isBatchMode ? (
          <>
            ...
            <div className="glass rounded-3xl p-6 border-pink-100 dark:border-pink-900/30 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-end mb-4 relative z-10">
                <div>
                  <h4 className="text-[10px] font-bold uppercase text-pink-400 tracking-widest">
                    Arrival Progress
                  </h4>
              ...
              <div className="h-4 bg-pink-50 dark:bg-pink-900/20 rounded-full overflow-hidden relative border border-pink-100 dark:border-pink-900/50 z-10">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${arrivalProgress}%` }}
                  className="absolute inset-y-0 left-0 bg-gradient-to-r from-pink-400 to-pink-600 dark:from-pink-500 dark:to-pink-700 shadow-sm"
                />
              </div>
            </div>
          </>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="glass rounded-3xl p-4 md:p-5 flex flex-col items-center justify-center text-center space-y-1 border-pink-100 dark:border-pink-900/30">
              <span className="text-[10px] font-bold uppercase text-pink-400">
                Total Items
              </span>
              <span className="text-2xl md:text-3xl font-black text-gray-800 dark:text-gray-100">
                {totalItems}
              </span>
            ...
            <div className="glass rounded-3xl p-4 md:p-5 flex flex-col items-center justify-center text-center border-pink-100 dark:border-pink-900/30 relative overflow-hidden">
              <motion.div
                initial={{ height: 0 }}
                animate={{ height: `${arrivalProgress}%` }}
                className="absolute inset-x-0 bottom-0 bg-pink-500/10 dark:bg-pink-400/10 z-0"
              />
              <span className="text-[10px] font-bold uppercase text-pink-500 relative z-10">
                Progress
              </span>
              <span className="text-3xl font-black text-pink-600 relative z-10">
                {arrivalProgress}%
              </span>
            </div>
          </div>
      ...
      <div className="space-y-4">
        ...
        {isLoading ? (
          <div className="glass rounded-3xl p-12 flex flex-col items-center justify-center text-pink-400/50 dark:text-pink-400/30 animate-pulse">
          ...
          <div className="glass rounded-3xl p-12 flex flex-col items-center justify-center text-center border-dashed border-pink-200 dark:border-pink-900/30 bg-pink-50/20 dark:bg-pink-900/5 h-64 gap-3">
            <Search
              size={36}
              className="text-pink-400/60 dark:text-pink-400/30 mb-2"
            />
            <h3 className="text-lg font-black uppercase tracking-widest text-pink-500 dark:text-pink-400">
              Search to View Orders
            </h3>
            <p className="font-bold text-[11px] uppercase tracking-wider text-gray-400 dark:text-gray-500">
              Type your @username or #tag above to reveal your items! ✨
            </p>
          </div>
        ) : displayedOrders.length === 0 ? (
          <div className="glass rounded-3xl p-10 flex flex-col items-center text-center text-gray-500 border-dashed border-pink-200 dark:border-pink-900/50">
            <SearchX size={32} className="text-pink-300 mb-4 opacity-50" />
            <p className="italic text-sm">No items match your search ✨</p>
          </div>
        ) : (
          <div className="glass rounded-3xl shadow-xl shadow-pink-100/50 dark:shadow-none border border-pink-100 dark:border-pink-900/30 overflow-hidden flex flex-col">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-pink-500 text-white text-[10px] uppercase font-black tracking-widest leading-none">
                      <th className="px-5 py-5 rounded-tl-2xl whitespace-nowrap">
                        Tag
                      </th>
                      <th className="px-5 py-5 whitespace-nowrap">User</th>
                      <th className="px-5 py-5 whitespace-nowrap">
                        Specification
                      </th>
                      <th className="px-5 py-5 text-center whitespace-nowrap">
                        Qtt
                      </th>
                      ...
                      <th className="px-5 py-5 rounded-tr-2xl text-center whitespace-nowrap">
                        Link
                      </th>
                    </tr>
                  </thead>
                    ...
                    {currentItems.map((order, idx) => {
                      ...
                      const hasLink =
                        ...
                        (linkVal.startsWith("http") ||
                          linkVal.startsWith("t.me"));

                      return (
                        <tr
                          key={idx}
                          className="hover:bg-pink-50/50 dark:hover:bg-pink-900/10 transition-colors text-[12px]"
                        >
                          ...
                          <td className="px-4 py-4 whitespace-nowrap font-bold text-gray-700 dark:text-gray-200">
                            {order.USERNAME?.startsWith("@")
                              ? order.USERNAME
                              : `@${order.USERNAME}`}
                          </td>
                          <td className="px-4 py-4">
                            <p className="text-gray-800 dark:text-gray-200 font-medium leading-tight">
                              {toTitleCase(order.SPECIFICATION) || "—"}
                            </p>
                          </td>
                          <td className="px-4 py-4 text-center font-bold text-gray-500 dark:text-gray-400">
                            {order.QTT || "1"}
                          </td>
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-center">
                            <span
                              className={`px-2 py-0.5 rounded-xl text-[10px] font-black uppercase border shadow-sm whitespace-nowrap inline-block min-w-[100px] ${statusColors(order.STATUS)}`}
                            >
                              {order.STATUS}
                            </span>
                          </td>
                          <td className="px-4 py-4 text-center">
                            {hasLink ? (
                              <a
                                ...
                                className="group inline-flex p-1.5 bg-pink-50 dark:bg-pink-950/20 text-pink-500 rounded-lg hover:bg-pink-500 hover:text-white transition-all active:scale-90 shadow-sm"
                              >
                                <ExternalLink
                                  size={14}
                                  className="group-hover:rotate-12 transition-transform"
            ...
            <div className="bg-pink-50/50 dark:bg-pink-950/20 px-6 py-4 border-t border-pink-100 dark:border-pink-900/50 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <label className="text-[10px] font-black uppercase text-pink-400 tracking-widest">
                  Per Page:
                </label>
                <select
                  value={itemsPerPage}
                  ...
                  className="bg-white dark:bg-pink-900/30 border border-pink-100 dark:border-pink-800 px-2 py-1 rounded-lg text-xs font-bold text-pink-500 outline-none"
                >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded-xl border border-pink-100 dark:border-pink-900/50 bg-white dark:bg-pink-900/20 text-pink-500 disabled:opacity-30 disabled:pointer-events-none hover:bg-pink-50 transition-all active:scale-90"
                >
                  <ChevronLeft size={16} />
                </button>
                <div className="px-4 py-1.5 bg-pink-500 text-white rounded-xl text-xs font-black shadow-md shadow-pink-200/50 dark:shadow-none">
            ...
            <div className="bg-pink-50/10 dark:bg-pink-950/30 px-6 py-3 border-t border-pink-100 dark:border-pink-900/30 flex flex-wrap items-center gap-4 text-[9px] text-gray-400 dark:text-gray-500 font-black uppercase tracking-widest">
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-green-500 shadow-sm shadow-green-200/50 dark:shadow-none" />{" "}
                Paid
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-red-400 shadow-sm shadow-red-100/50 dark:shadow-none" />{" "}
                Unpaid
              </div>
              <div className="ml-auto text-pink-400/80 flex items-center gap-1">
                <Info size={12} /> Live Sync Active
              </div>
            </div>
          </div>
        )}
